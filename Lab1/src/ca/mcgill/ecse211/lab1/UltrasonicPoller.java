...
public class UltrasonicPoller implements Runnable {
  ...
  public UltrasonicPoller() {
    usData = new float[US_SENSOR.sampleSize()];     // create an array of float of size corresponding to the number of elements in a sample. The number of elements does not change during runtime.
    controller = Main.selectedController;           // create a ultrasonic controller 
  }

  /*
   * Sensors now return floats using a uniform protocol. 
   * Need to convert US result to an integer
   * [0,255] (non-Javadoc)
   * 
  ...
  public void run() {
    int distance;
    while (true) {
      US_SENSOR.getDistanceMode().fetchSample(usData, 0); // acquire distance data in meters and store it in usData (an array of float)
      distance = (int) (usData[0] * 100.0); // extract from buffer (region of a physical memory storage used to temporarily store data while it is being moved from one place to another), convert to cm, cast to int
